"""Weather Agent FastAPI server - exposes POST /task for supervisor."""
...
async def handle_task(req: TaskRequest):
    ...
    logger.info(f"Weather Agent received task: {req.task[:100]}...")
    result = await run_task(full_task)
    logger.info(f"Weather Agent completed with status: {result['status']}")
